#!/usr/bin/env bash
# 시연영상 촬영 런처 — 한 번 실행에 영상 3개(GUI · 1인칭 오버레이 켬/끔)를 동시에 남긴다.
#
# 설계 = 상위 docs/superpowers/specs/2026-09-03-시연영상-촬영-design.md
#
# 🔴 평소 실행(바탕화면 「SOP 가디언 Demo」)과 다르다. 저쪽은 녹화를 켜지 않는다.
# 3인칭 웹캠 녹화·「웹캠만」 회차는 2026-09-23 제거(백업 태그 backup/webcam-before-removal-20260923).
#
...
echo "    화면이 뜨고 카메라 영상이 들어오면 이 회차의 녹화 3개가 동시에 시작됩니다."
echo "    끝나면 GUI 창을 닫으세요."
echo

SOP_DEMO_CAPTURE=1 SOP_DEMO_SCENARIO="$SCEN" SOP_DEMO_OVERLAY="$OVL" \
SOP_DEMO_HIDE_VIDEO="$HIDE" SOP_DEMO_TARGETS="$TARGETS" python3 main.py
status=$?
