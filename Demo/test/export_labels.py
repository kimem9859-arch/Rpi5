"""console_v1의 검출 결과(rawdet_log.csv)를 YOLO 라벨(.txt)로 내보낸다 — 프리라벨.

왜 필요한가:
    console_v1은 ESP32 영상에서 B1·B2·B3·EMO를 잘 잡는다(50~70% 프레임).
    그 검출을 **초벌 라벨**로 쓰면 사람이 처음부터 5개를 다 그릴 필요가 없다.
    남는 수작업은:
      - **B4(파랑 스티커)** — v1은 검정 버튼으로 학습돼 못 잡는다. 전량 수동.
      - **놓친 버튼** — 저조도 세션은 B3가 0%다. 채워 넣어야 한다.
      - **정반사 오분류** — 색이 날아가면 B1·B3를 B2로 잡는다. 고쳐야 한다.

⚠️ 프리라벨을 검수 없이 그대로 학습에 쓰면 v1의 오류를 그대로 물려받는다.
   특히 정반사·저조도 세션은 **반드시 눈으로 확인**할 것.
...
def main():
    ...
    for c in CLASS_NAMES:
        note = ""
        if c == "B4":
            note = "  ← 파랑 스티커. v1이 못 잡음 → **전량 수동 라벨 필요**"
        elif stats[c] < n * 0.5:
            note = "  ⚠️ 프레임의 절반도 못 잡음 → 수동 보충 필요"
    ...
    print("\n" + "=" * 60)
    print("다음 단계 — 라벨링 툴(labelImg·Roboflow)에서 반드시 검수할 것:")
    print("  1. B4(파랑) 박스를 **모든 프레임에** 추가")
    print("  2. 정반사 세션: B1·B3가 B2로 잘못 잡힌 것 수정")
    print("     ※ 정답은 **진짜 버튼 정체**다. 노란 버튼이 하얗게 보여도 B1.")
    print("  3. 저조도 세션: 놓친 B3·EMO 채워넣기")
    print("=" * 60)
