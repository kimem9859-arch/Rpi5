...
#       효과가 묻힌다. USB 웹캠은 2026-09-23 제거 — 재측정은 ESP32 뿐이고 그 한계를 함께 적는다.
...
DETECT_BOX_COLORS = {
    ...
    "B2":  "#FFFFFF",   # 흰
    "B3":  "#FF69B4",   # 핑크
    "B4":  "#6BB6FF",   # 밝은 파랑 (파랑 원 스티커, 통합문서 §10.12) — 어두운 배경에서 #2E7DFF 는 검정처럼 묻혔다
    "EMO": "#FF3B30",   # 빨강 (비상정지)
}
...
# 디바운스(기계식 채터링 제거). 🔴 gpiozero(lgpio) 디바운스는 「무시 시간」이 아니라 **유지 시간** 이다 —
#    신호가 이 시간 동안 안정돼야 알린다. 그래서 값보다 짧게 친 누름은 **사라지고**, 모든 누름·EMO 신호가
#    그만큼 **늦는다**(EMO 해제 시 레벨 재확인 is_pressed 는 영향 없음 · lgpio gpio_set_debounce_micros 원문).
# ⚠️ 2026-09-05 결선에서 한 번 누름이 3~4회 기록됐다 — 0.05초 안정 신호가 반복됐다면 채터링보다 배선
#    (풀업·긴 선 잡음·GND 공유) 쪽이 유력. 0.2 임시 확정(09-11)은 **보류**하고 0.05 유지(2026-09-15 사용자 결정).
#    결선 날 판정: 배선 점검 먼저 → B1~B4 각 「보통 누름 5회 + 짧게 톡 5회」 → 로그 줄 수 · 0.05/0.1/0.2 비교.
