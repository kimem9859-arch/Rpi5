...
⚠️ **스케줄러를 쓰면 `network_group.activate()` 를 직접 호출하지 않는다.**
   ...
   ✅ **해결됨 (2026-08-05)** — 종전에는 손 추론이 도는 **프레임마다** 이 경고가 났다:
   ...
   경고 32건 → 0건, 손 검출 결과는 5장으로 동일).
   ⚠️ **HailoRT 5.x 로 올릴 때는 여전히 그 소스를 봐야 한다** — 감싸개는 경고를
      없앨 뿐 upstream 코드를 고치지 않는다. 그때는 `.hef` 재변환(현행 .hef 는
      DFC 3.33.1 산출물이라 4.x 전용)·blaze vendoring 과 **묶어서** 처리한다.
