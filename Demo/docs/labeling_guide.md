...
**③ 거의 안 보이거나(⅓ 이하) 무엇인지 확신이 없으면 → 그리지 않는다.** 추측해서 그리지 않는다. **흔들림 잔상으로 물체가 두 개로 겹쳐 보이거나 번져서 알아볼 수 없을 때도** 그리지 않는다.
...
| **확신 못 하는 물체만 보인다**(잔상·번짐으로 알아볼 수 없는 경우 포함) | 박스 없이 **태그 `exclude`** + 코멘트에 이유 한 줄 |
