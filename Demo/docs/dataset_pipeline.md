...
이때 **B3(핑크)가 완전히 죽는다**(§12.13). 극단 조건은 물리적으로 만들기보다 **증강으로 합성**하는 편이 통제하기 쉽다.
...
| 필요 | Roboflow | 근거 |
|---|---|---|
| 블러 σ0~2.0 | 있음 | B4 사멸점 σ0.8 (§12.9) |
| **JPEG 압축 q30~100** | **없음** | B4 사멸점 q50 |
| **저해상도 다운스케일** | **없음** | 5% 축소로도 사멸 |
| **정반사 하이라이트 합성** | **없음** | B1·B3 → B2 오분류 (§12.8) |
...
~~**🔴 test 세션 미촬영.**~~ → ✅ **해소(2026-07-20)** — 아래 §8 참조.

---

## 8. test 세션 촬영·배분 (2026-07-20) ⭐

**촬영 완료.** 클린룸(다른 날·다른 장소·다른 조명 종류)에서 5세션 촬영. 상세·수치 정본 = 상위 통합문서 **§12.18·§12.19**.

| 항목 | 값 |
|---|---|
| 촬영 | `cleanroom-fluorescent` 5세션 · 1,500프레임 (`--raw-every 1`) |
| **중복 제거 후** | **495장** (67% 제거) |
| 배분 | **test 130 / train 292 / valid 73** |
| Roboflow | `console_v2-mjefr`에 split 명시 업로드 완료(중복·오류 0) |

배분 상세·취급 규칙 = `dataset/cleanroom_eval_20260720/ALLOCATION.md`.

### 이번에 §4와 다르게 한 것 — 세션 단위가 아니라 세션마다 나눠 할당

§4는 "분할은 세션 단위로"라고 했으나, 그것은 **같은 조건 세션이 여럿일 때**의 누출 방지책이다. 이번 5세션은 **각각 다른 촬영 변화 축**(좌우+위아래각도 / 원근거리 / 가림 / 카메라+콘솔회전 / 좌우각도)이라, 세션을 통째로 test에 주면 **그 변화 축이 학습에서 완전히 빠진다.** → 세션마다 **균등 간격**으로 test를 뽑고 나머지를 train/valid로 돌렸다.

### ⚠️ test는 프리라벨(§3)을 쓰지 않는다

§3의 프리라벨은 **모델 검출을 라벨로 재사용**하는 것이다. v2는 형광등에서 B4까지 잘 잡으므로 이제 v2 프리라벨이 가능하고 재추론조차 필요 없다(오늘 rawdet 로그가 곧 v2 검출).

**그러나 test에는 절대 쓰지 않는다** — 평가 대상 모델로 정답을 만들면 모델의 오류를 사람이 "정답"으로 승인하게 되어 채점이 순환논리가 된다. **test는 전량 수동 라벨링.** train/valid는 학습 데이터라 프리라벨을 써도 무방하다.

### 🔴 남는 한계
train/valid와 test가 **같은 세션**에서 나왔다. 따라서 **추후 v3 재학습 후의 채점은 낙관적**이다. 정식으로 "클린룸에서 쓸 수 있다"를 주장하려면 **재학습 시점에 새로 촬영한 test 세션**이 필요하다.
※ **현재 v2 채점에는 이 한계가 없다** — v2 학습분은 7/13 촬영 652장뿐이고 클린룸 이미지는 한 장도 들어가지 않았다.
