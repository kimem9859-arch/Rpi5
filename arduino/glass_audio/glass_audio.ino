/*
 * 서브 글라스 — **오디오 전용** (마이크 + 스피커, 무선)
 *
 * 정본 = docs/superpowers/specs/2026-09-06-음성비서-시연구현-design.md
 *
 * 🔑 왜 보드를 나눴나 (2026-09-06 사용자 결정) — 한 보드에 카메라·마이크·스피커를
 *    다 얹으면 **G1(셋 동시 초기화)이 미검증 위험**으로 남는다. 촬영을 이틀 앞두고
 *    거기서 막히면 대안을 세울 시간이 없다. 나누면 **각각은 이미 검증된 조합**이다.
 *      메인 ESP32 = 카메라만 (`camera_stream_tcp` — 변경 0줄)
 *      서브 ESP32 = 이 스케치 (마이크·스피커)
 *
 * 🔑 서브 보드는 **카메라만 불량한 예비 본체**를 쓰면 딱 맞는다 — 마이크·I2S·WiFi 는
 *    정상임이 확인돼 있다(§10.55-(5)).
 *
 * 포트: 8889 마이크 업링크 · 8890 명령/스피커   (카메라 8888 은 메인 보드 몫)
 *
 * 🔴 mDNS 를 켜지 않는다 — 메인과 이름이 겹치면 파이가 어느 쪽에 붙을지 알 수 없다
 *    (§10.55-(6) · 통신경로 설계 §6-② 가 mDNS 채택을 철회했다).
 *    파이는 `Demo/.audio_ip` 에 적힌 주소로 붙는다.
 *
 ...
 *
 * 🔴 굽기 — PSRAM=opi 필수(오디오 버퍼 480KB 를 PSRAM 에 잡는다):
 *   arduino-cli compile --fqbn esp32:esp32:XIAO_ESP32S3:PSRAM=opi arduino/glass_audio
 *   arduino-cli upload -p <포트> --fqbn esp32:esp32:XIAO_ESP32S3:PSRAM=opi arduino/glass_audio
 */
