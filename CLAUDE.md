...
4. **▶ 최우선 = ⑤ replay 평가 (파이에서 수행)** — **B4 판정은 오직 여기서만 가능하다.**
   ...
   ```
   - 🔴 **판정 기준 정본 = 통합문서 §10.16. "v1 대비 검출률 상승"을 기준으로 쓰지 말 것**(2026-07-16 정정) — `test/raw/<세션>`이 곧 **652장 학습 데이터의 출처**라(`Demo/dataset_pipeline.md`) **학습에 쓴 이미지로 시험 보는 구조**다. 게다가 v1은 **파랑 스티커를 학습한 적이 없어**(§10.12 "v1으론 여전히 0% — 학습 분포 밖, **당연**") 0%가 예정돼 있고 v2는 이 프레임으로 학습했다. → **"v1 0회 → v2 N회"는 자동으로 나오고 아무것도 증명하지 않는다.**
   - ✅ **대신 이것만 본다**:
     ...
     3. **`.hef` vs `.pt` 대조** — 같은 프레임에 둘 다 돌려 **양자화 손실만** 분리(변인 통제).
   ...
   - ⑤ 통과 시 → `config.HEF_MODEL_PATH`를 `console_v2.hef`로 전환 + 결과를 통합문서 §10에 기록(수치 정본).
