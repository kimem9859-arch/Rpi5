"""오탐 검수용 사진 만들기 — 저장된 raw PNG 에 「실제 시연 프로그램이 판정에 쓴 박스」만 그린다(R2 전 추적 기준 — 아래 ⚠️).
...
⚠️ 인식 수정 ③ R2(2026-09-25) 뒤로 `_update_tracks` 는 **한 프레임짜리 미확정 트랙**(판정·표시에 안 씀)도
   돌려주고, 같은 클래스 고르기 순서도 바뀌었다(지금 보임 → 오래됨 → 점수). 이 스크립트는 `miss == 0` 만 보므로
   미확정 트랙도 그린다 — 저장된 사진(R2 전 추적)과 똑같이 다시 만들어지지 않는다. 지금의 판정 박스만 보려면
   `t["confirmed"]` 도 거른다(그러면 저장된 사진과 더 달라진다).
"""
import argparse
import collections
import csv
